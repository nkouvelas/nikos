...
/* The weighing function is symetric; half the weighs are recorded */
	...
	for (uint16_t i = 0; i < (samples >> 1); i++) {
		...
		case FFT_WIN_TYP_WELCH: /* welch */
			weighingFactor = 1.0 - (((indexMinusOne - samplesMinusOne / 2.0) / (samplesMinusOne / 2.0))*((indexMinusOne - samplesMinusOne / 2.0) / (samplesMinusOne / 2.0)));
			break;
		}
		if (dir == FFT_FORWARD) {
...
//double PlainFFT::MajorPeak(double *vD, int samples, double samplingFrequency) //ithele *
//{
	//double maxY = 0;
	...
	//double interpolatedX = ((IndexOfMaxY + delta)  * samplingFrequency) / (samples-1); //htan double
